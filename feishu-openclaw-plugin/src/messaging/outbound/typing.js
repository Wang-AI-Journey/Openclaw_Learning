...
// ---------------------------------------------------------------------------
/**
 * 规范化 message_id，处理合成的 ID
 * - "om_xxx:auth-complete" → "om_xxx"
 * - "synthetic_xxx_account" → 返回 null（跳过 typing）
 */
function normalizeMessageId(messageId) {
    // synthetic ID 无法用于飞书 API，直接返回 null 跳过 typing
    if (messageId.startsWith('synthetic_')) {
        return null;
    }
...
export async function addTypingIndicator(params) {
    ...
    // 规范化 message_id，处理合成 ID
    const normalizedId = normalizeMessageId(messageId);
    const state = {
        messageId: normalizedId, // 保存规范化后的 ID
        reactionId: null,
    };
    // synthetic ID 无法用于飞书 API，直接跳过 typing
    if (!normalizedId) {
        console.debug(`[feishu-typing] Skip typing indicator for synthetic message ID: ${messageId}`);
        return state;
    }
    try {
